...
class AlumnoNegocio():
    ...
    def eliminar_alumno(self, indice):
        if 0 <= indice < len(self.listado_alumno):
            alumno_eliminado = self.listado_alumno.pop(indice)
            print(f'Se eliminó el alumno: {alumno_eliminado.nombre}')
            self.guardar_alumnos()
            return True
        else:
            print('Índice fuera de rango. No se pudo eliminar el alumno.')
            return False